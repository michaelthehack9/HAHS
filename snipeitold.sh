...
while true; do
  ...
  # === Lookup full asset info ===
  ASSET_URL="$SNIPEIT_BASE_URL/api/v1/hardware?search=$ASSET_TAG"
  RESPONSE=$(curl -s -H "$AUTH_HEADER" -H "$ACCEPT_HEADER" -H "$CONTENT_HEADER" "$ASSET_URL")

  TOTAL=$(echo "$RESPONSE" | jq '.total')
  if [ "$TOTAL" -eq 0 ]; then
    echo "❌ No asset found with tag '$ASSET_TAG'"
    continue
  fi

  ASSET=$(echo "$RESPONSE" | jq -r --arg TAG "$ASSET_TAG" '.rows[] | select(.asset_tag == $TAG)')
  if [ -z "$ASSET" ]; then
    echo "❌ Asset tag '$ASSET_TAG' not found in search results."
    continue
  fi

  ASSET_ID=$(echo "$ASSET" | jq '.id')
  MODEL_NAME=$(echo "$ASSET" | jq -r '.model.name')
  ASSIGNED_TO=$(echo "$ASSET" | jq -r '.assigned_to.name // "Not assigned"')
